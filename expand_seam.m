...
%
%   Example:
%         img_out = expand_seam(img, seam, 'vertical')
%
...
%% Initialization
img_out = zeros(r,c+1,d); % Initialize imgOut.

%% Expand pixels on the seam
for i = 1:r
    for k = 1:d
        currentLineLeft = img(i,1:seam(i),k);
        currentLineRight = img(i,seam(i)+1:end,k);
        img_out(i,:,k) = [currentLineLeft, currentLineLeft(end), currentLineRight];
    end
end
